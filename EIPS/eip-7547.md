...
The parent transactions addresses contract reverts whenever called by a `caller` other than `SYSTEM_ADDRESS`. It has two operations: `set` and `get`. The input is used to determine which operation is performed: if it is empty, 'get' is performed, otherwise 'set' is performed.

##### `set`

* Caller sets the first 32 bytes as the number of addresses that are passed, and the rests of the input is a concatenation of the addresses
* Set the storage value at 0 to be `evm.calldata[0:32]`, the number of addresses that are passed
* For each address, i.e., every following chunk of 20 bytes in evm.calldata, set the first available storage slot to that

##### `get`

* Caller does not pass any calldata
...
def set():
    storage.set(0, evm.calldata[0:32])
    num_addresses = to_uint256_be(evm.calldata[0:32])
    for i in range(num_addresses):
        start_index = 32+20*i
        storage.set(i+1, evm.calldata[start_index:start_index+20])
```
